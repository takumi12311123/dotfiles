...
Step 2: 既存資産の調査  → 再利用できるものを探す (無いことも根拠つきで示す)
...
**新規実装は最後の手段。** 差分が毎回膨らむのは、設計が悪いからではなく
「既にあるものを探さずに書き始めるから」であることが多い。

### 探し方（4 経路。1 つで済ませない）

```bash
# 1. 名前で探す — 機能名・ドメイン語・動詞を変えて数回引く
grep -rin '<キーワード>' --include='*.<ext>' . | head -30

# 2. 構造で探す — シンボル定義から辿る
#    serena: get_symbols_overview → find_symbol → find_referencing_symbols

# 3. 呼び出し側から探す — 似た処理をしている既存の呼び出し元を読む
#    「この機能を使うのは誰か」を先に決めると、その隣に既存実装がいることが多い

# 4. 依存で探す — 新しい依存を足す前に、既に入っている依存で足りないか
cat package.json go.mod Cargo.toml pyproject.toml 2>/dev/null
```

外部ライブラリを新たに使うなら `latest-docs` で現行 API を確認する
（導入自体はユーザーの許可が要る / `.claude/rules/coding-principles.md`）。

### 判定と出力（spec の `## 既存資産の調査` 節になる）

**スコープの各項目について最低 1 行**書く。判定は `再利用` / `拡張` / `踏襲` / `新規` の 4 語で、
各判定に何を書く必要があるかは `.claude/rules/flow-gates.md` の Gate A 条件 3 が定義する
（そこに機械判定 `reuse_survey_ok()` もある。書いたら通るか確かめられる）。

```markdown
| 探した対象 | 見つかったもの | 判定 | 理由 |
|-----------|--------------|------|------|
| リトライ処理 | `internal/http/retry.go` の `WithRetry` | 再利用 | シグネチャがそのまま合う |
| 設定読み込み | `internal/config/config.go` の `Load` | 拡張 | 引数を 1 つ足せば足りる。呼び出し元 3 箇所は影響なし |
| バックオフ計算 | なし | 新規 | `grep -rin 'backoff\|jitter'` で 0 件、`internal/http/` も確認 |
```

**探索の痕跡なしに「新規」と書かない。** 「見つからなかった」は結論であって、
探していないことの言い換えではない。これは Gate A の成立条件（`.claude/rules/flow-gates.md`）。

調査で分かった **制約** も仕様に持ち込む（「この経路は同期でしか呼べない」等）。
...
| 既存を探さずに新規実装を前提に案を書く | 差分が毎回膨らむ。既存との二重管理が生まれる |
| 「該当なし」とだけ書いて探索の痕跡を残さない | 検証できない = 探していないのと区別がつかない |
| 既存を「拡張」するのに既存呼び出し元への影響を書かない | 他所を壊す変更が非スコープ扱いで滑り込む |
